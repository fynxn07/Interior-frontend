...
import { FaTimes, FaEnvelope, FaPaperPlane, FaCheckCircle, FaClock } from "react-icons/fa";
import ModalPortal from "./ModalPortal";

function MessageDetailModal({ message, open, onClose, onReply }) {
  const [replyText, setReplyText] = useState("");
  const [sending, setSending] = useState(false);

  useEffect(() => {
    if (message) setReplyText(message.reply || "");
  }, [message]);
  ...
  const handleSendReply = () => {
    ...
    // Opens the admin's real email client, pre-filled — genuine send today.
    const subject = encodeURIComponent(`Re: Your message to OK Decoration`);
                ...
                <div className="relative w-full max-h-[88vh] rounded-3xl border border-white/10 bg-[#131826]/95 backdrop-blur-2xl shadow-2xl shadow-[#6C5CE7]/10 overflow-y-auto overflow-x-hidden">
                  <div className="flex items-center justify-between px-6 sm:px-8 py-5 border-b border-white/10 sticky top-0 bg-[#131826]/95 backdrop-blur-2xl z-10">
                    ...
                    <a
                      ...
                      >
                        <FaEnvelope size={10} /> {message.email}
                      </a>
                    </div>
                    <button
                      onClick={onClose}
                  ...
                  <div className="p-6 sm:p-8 space-y-5">
                    ...
                    <div className="pt-4 border-t border-white/10">
                      ...
                      <textarea
                        ...
                        rows={5}
                        placeholder="Write your reply — this opens your email client to actually send it..."
                      ...
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-2 mt-4 py-3 rounded-xl bg-gradient-to-r from-[#8B7CFF] to-[#6C5CE7] text-white font-semibold hover:opacity-90 transition-opacity disabled:opacity-50"
                      >
                        <FaPaperPlane size={13} />
                        {sending ? "Opening email client..." : "Send Reply"}
                      </button>
