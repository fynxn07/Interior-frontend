...
function QuotationDetailModal({ quotation, open, onClose, onSave }) {
    ...
    useEffect(() => {
        if (quotation) {
            ...
            setAssignedTo(quotation.assigned_to || "");
            setSaved(false);
        }
    }, [quotation]);
    ...
    const mailSubject = encodeURIComponent(
        `Regarding your quotation ${quotation.reference}`
    );
...
We have received your quotation request and will contact you shortly.
...
OK Decoration`
    ...
    const replyLink =
        ...
        `&to=${encodeURIComponent(quotation.email)}` +
        `&su=${mailSubject}` +
        `&body=${mailBody}`;

    const handleSave = async () => {
        setSaving(true);
        await onSave(quotation.id, { status, price, remarks, assigned_to: assignedTo });
        setSaving(false);
        setSaved(true);
                                        ...
                                        <div className="lg:col-span-3 space-y-5 min-w-0">
                                            ...
                                            {quotation.preferred_date && (
                                                <div className="flex items-center gap-2.5 text-gray-400 text-sm">
                                                    ...
                                                    Preferred start: {new Date(quotation.preferred_date).toLocaleDateString()}
                                                </div>
                                            )}
                                        ...
                                        <div className="lg:col-span-2 space-y-5 min-w-0">
                                            <div className="rounded-xl border border-white/10 bg-white/[0.03] p-5 space-y-5">
                                                ...
                                                <a
                                                    ...
                                                    className="w-full flex items-center justify-center gap-2 py-3 rounded-lg border border-[#8B7CFF]/40 text-[#8B7CFF] hover:bg-[#8B7CFF]/10 transition-colors"
                                                >
                                                    <FaEnvelope size={13} />
                                                    Reply to Customer
                                                </a>
