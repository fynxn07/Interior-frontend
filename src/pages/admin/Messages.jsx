            ...
            <div className="flex flex-wrap gap-2.5 mb-6">
                {["All", "unread", "read", "replied"].map((s) => (
                    <button
                        key={s}
                        onClick={() => setFilter(s)}
                        className={`px-4 py-1.5 rounded-full text-xs uppercase tracking-wider border transition-all duration-300 capitalize ${filter === s
                ...
                <AnimatePresence mode="popLayout">
                    {filtered.map((m) => (
                        <motion.div
                            ...
                            className={`rounded-2xl border p-5 flex flex-col sm:flex-row sm:items-center gap-4 cursor-pointer transition-colors duration-300 ${m.status === "unread"
